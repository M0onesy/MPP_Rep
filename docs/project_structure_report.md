...
- 生成时间：2026-09-21T17:44:32+08:00
- 项目根目录：`D:\Competition\ZY_Project\选题五\mainProj`
- 生成脚本：`scripts/project_structure_report.py`，版本 `1.0.0`
- 扫描方式：只读取目录项、文件扩展名和文件大小，不加载原始 CSV、Parquet 或 PDF 正文。

## 一、总体判断

当前项目采用“原始数据 → 中间数据 → 成品数据 → 报告”的四段式结构，已经足够支撑当前 Python 研究原型。本报告不新增复杂的软件包层级，也不搬动超大原始文件；测试文件和辅助文件通过规范状态进行区分。
...
|---|---:|---:|---|---|
| `data/raw/` | 27 | 154.48 GiB | .csv: 20, .pdf: 5, .py: 2 | 原始供应商文件与字段字典 |
| `data/mid/` | 92 | 3.26 GiB | .parquet: 92 | 脚本生成的中间数据 |
| `data/prod/` | 26 | 278.59 MiB | .csv: 18, .json: 4, .parquet: 4 | 分析成品与原型输出 |
| `scripts/` | 18 | 503.82 KiB | .py: 9, .pyc: 9 | 处理与报告生成脚本 |
| `docs/` | 5 | 354.99 KiB | .md: 5 | 说明文档 |
| `reports/figures/` | 17 | 2.68 MiB | .png: 17 | 生成图表 |
...
│   │   ├── test_proto2/
│   │   ├── test_proto3/
│   │   ├── crsp_fee_by_size_group.csv
│   │   ├── crsp_fee_descriptive.csv
│   │   ├── crsp_fee_timeseries.csv
│   │   ├── crsp_markit_matching.csv
│   │   ├── crsp_markit_matching_summary.csv
│   │   ├── crsp_monthly_panel.parquet
│   │   ├── crsp_panel_metadata.json
│   │   ├── markit_coverage.csv
│   │   ├── markit_coverage.json
│   │   ├── markit_fee_descriptive.csv
│   │   ├── markit_fee_persistence.csv
│   │   ├── markit_fee_quality.csv
│   │   ├── markit_fee_timeseries.csv
│   │   ├── markit_fee_variance_decomposition.csv
│   │   ├── markit_stock_monthly.parquet
│   │   ├── prototype_constraint_check.csv
│   │   ├── prototype_metadata.json
│   │   ├── prototype_monthly_metrics.csv
│   │   ├── prototype_sensitivity.csv
│   │   ├── prototype_summary.csv
│   │   └── prototype_weights.parquet
│   └── raw/
│       ├── 13f/
│       ├── 属性/
│       ├── 日频/
...
│   ├── raw_data_dictionary_and_reproduction_status.md
│   ├── raw_data_inventory_report.md
│   └── research_pipeline_readme.md
├── reports/
│   └── figures/
│       ├── markit/
│       ├── presentation/
...
└── scripts/
    ├── __pycache__/
    │   ├── build_crsp_panel.cpython-312.pyc
    │   ├── make_presentation_visuals.cpython-312.pyc
    │   ├── make_research_reports.cpython-312.pyc
    │   ├── markit_coverage.cpython-312.pyc
    │   ├── pipeline_utils.cpython-312.pyc
    │   ├── project_structure_report.cpython-312.pyc
    │   ├── raw_data_dictionary_report.cpython-312.pyc
    │   ├── raw_data_inventory.cpython-312.pyc
    │   └── run_m0_m3_prototype.cpython-312.pyc
    ├── build_crsp_panel.py
    ├── make_presentation_visuals.py
    ├── make_research_reports.py
    ├── markit_coverage.py
    ├── pipeline_utils.py
    ├── project_structure_report.py
    ├── raw_data_dictionary_report.py
    ├── raw_data_inventory.py
    └── run_m0_m3_prototype.py
```
